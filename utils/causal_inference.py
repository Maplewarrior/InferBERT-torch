...
def causal_tree(feature_file, probability_file, threshold, condition_list):
    ...
    ### the below code assumes that index is the first 
    probability_df = pd.read_csv(probability_file)
    probability_dict = {i + 1 : probability_df.iloc[i]['probability'] for i in range(len(probability_df))}
    ...
    def compute_gap(feature_dict, p_dict, not_empty_dict):
        ...
        def compute_Z_score(in_list, out_list):
            ...
            for term in in_list:
                ...
                L1.append(float(term))
            L2 = []
            for term in out_list:
                L2.append(float(term))
...
def main():
    import time
    config = {'probability_file': 'experiments/reproduction/outputs/liverfailure/probability_file.csv',
              'feature_file': 'data/reproduction/Analgesics-induced acute liver failure/processed/feature.csv',
              'out_dir': 'experiments/reproduction/outputs/liverfailure/causality_output'}
    start = time.time()
    print(f'Running Causal Inference!')
    causal_inference(config['probability_file'], config['feature_file'], config['out_dir'])
    end=time.time()
    print(f'Causal inference done! Elapsed: {end-start}')
    generate_causal_tree(config['probability_file'], config['feature_file'], config['out_dir'])
    end=time.time()
    print(f'Causal tree done! Elapsed: {end-start}')
