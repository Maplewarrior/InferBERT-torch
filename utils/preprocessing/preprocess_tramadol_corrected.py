...
"""
Update to tramadol preprocessing removing Completed suicide and Death from ADE features so as to not convey the label.
Hence, this preprocessing script is not part of reproduction!
"""
...
def _generate_ALBERT_dataset(dict, target_list, out_dir, index_dict):
    def _generate_dataset(dict, target_list, index_dict):
        ...
        for inx, case in enumerate(value_list):
            ...
            ### clean ADE
            ade_stopwords = ['Death', 'death', 'Completed suicide']
            ade = ','.join([e for e in ade.split(',') if e.strip() not in ade_stopwords])
